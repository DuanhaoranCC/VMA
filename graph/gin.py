...
from torch.nn import Sequential, Linear, ReLU, PReLU, ELU
from torch_geometric.datasets import TUDataset
from torch_geometric.data import DataLoader
from torch_geometric.nn import GINConv, global_add_pool, LayerNorm, BatchNorm
...
class Encoder(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers, p1, p2, p3):
        ...
        for i in range(num_gc_layers):
            ...
            if i:
                nn = Sequential(Linear(dim, dim), self.action, torch.nn.Dropout(p=p1),
                                Linear(dim, dim))
            else:
                nn = Sequential(Linear(num_features, dim), self.action, torch.nn.Dropout(p=p1),
                                Linear(dim, dim))
            conv = GINConv(nn)
            bn = BatchNorm(dim)
            ...
            self.bns.append(bn)
        self.dp1 = torch.nn.Dropout(p=p2)
        self.dp2 = torch.nn.Dropout(p=p3)
