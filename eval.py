...
def train_test_split1(labels, seed, train_examples_per_class=None, val_examples_per_class=None,
...
def label_classification(embeddings, train_mask, val_mask, test_mask, label, type, device, name='common'):
    ...
    if type == 1:
        # mask = train_test_split1(
        #     label.detach().cpu().numpy(), seed=0, train_examples_per_class=20,
        ...
        encoder.to(device)
        optimizer_f = AdamW(encoder.parameters(), lr=0.01, weight_decay=1e-4)
